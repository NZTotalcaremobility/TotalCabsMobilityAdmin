...
import { DateOfBirth, OnlyCharFieldValidator } from 'src/app/shared/validation/validations.validator';
import { environment } from 'src/environments/environment';
import { UserService } from '../../user/user.service';

@Component({
  selector: 'app-edit-companyuser',
  templateUrl: './edit-companyuser.component.html',
  styleUrls: ['./edit-companyuser.component.scss']
})
export class EditCompanyuserComponent implements OnInit {
  ...
  createdDoctorRegForm() {
    ...
    this.userFormTab1 = this.fb.group({
      ...
      'phonenumber': ['', Validators.compose([Validators.required, Validators.pattern(/^[0][1-9]\d{6}$|^[1-9]\d{5,9}$/)])],
     // 'dob': ['', Validators.compose([Validators.required, DateOfBirth.dobValidator])],
    ...
    this.userFormTab2 = this.fb.group({
      ...
      'postcode': ['', Validators.compose([Validators.required, Validators.pattern(/^[0][1-9]\d{9}$|^[1-9]\d{3}$/)])],
  ...
  getUserById() {
    ...
    this.service.getUserById(_id).subscribe(res => {
      ...
      if (res) {
        ...
        this.userFormTab1.controls['companyname'].setValue(this.objData.companyname);
  ...
  submitForm() {
    if (this.userFormTab1.valid && this.userFormTab2.valid) {
      ...
      this.service.editcompanyuser(formModel).subscribe(res => {
        this.isDisabled = false
        if (res && res.code == 200) {
          this.navRoute.navigate(['/company']);
          this.reset();
          this._toastr.success("User Updated Successfully ");
        } else {
          this._toastr.info("Error", "Something Wrong");
        }
      }
      );
  ...
  private prepareSave(): any {
    ...
    inputData.append('companyname', this.userFormTab1.get('companyname').value);
